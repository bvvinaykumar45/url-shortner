...
import {
  ...
  updateUrlPatchRequestBodySchema,
...
export const updateUrlController = async (req, res) => {
  ...
  const validationResult = await updateUrlPatchRequestBodySchema.safeParseAsync(req.body);
  ...
  if (code) {
    updateSetValues.shortCode = code;
  }

  const updatedUrlRecord = await updateUrlById(id, userId, updateSetValues);

  return res.status(200).json(updatedUrlRecord);
};
